...
// ================= ENDPOINT PARA CONFIGURAR DOMINIO PERSONALIZADO (CLOUDFLARE) =================
app.post('/api/domains/provision', async (req, res) => {
    try {
        const authHeader = req.headers.authorization;
        if (!authHeader) return res.status(401).json({ error: 'No autorizado' });
        const token = authHeader.split(' ')[1];
        ...
        const { domainName, landingId, serverTargetIP } = req.body;

        if (!domainName || !landingId) {
            return res.status(400).json({ error: 'Faltan datos obligatorios (domainName o landingId)' });
        }

        // Verificar si el dominio ya está registrado en la base de datos
        const existingDomain = await Domain.findOne({ domainName });
        if (existingDomain) {
            return res.status(400).json({ error: 'Este dominio ya se encuentra registrado o en proceso.' });
        }
        ...
        // 1. Crear la Zona en Cloudflare vía API oficial v4
        ...
        if (!zoneData.success) {
            console.error('Error Cloudflare Zone:', zoneData.errors);
            return res.status(400).json({ 
                error: 'No se pudo crear la zona en Cloudflare', 
        ...
        // 2. Configurar el registro A principal apuntando a la IP de tu servidor
        const targetIP = serverTargetIP || process.env.SERVER_TARGET_IP || '127.0.0.1';
        ...
        await fetch(`https://api.cloudflare.com/client/v4/zones/${zoneId}/dns_records`, {
            ...
            body: JSON.stringify({
                ...
                content: targetIP,
                ttl: 1,
                proxied: true // Activa CDN, seguridad y SSL automático
            })
        });

        // 3. Configurar el registro CNAME para las variantes con 'www'
        await fetch(`https://api.cloudflare.com/client/v4/zones/${zoneId}/dns_records`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${cfToken}`,
        ...
        // 4. Guardar en MongoDB la relación del dominio
        const newDomainRecord = new Domain({
            domainName,
            userEmail: decoded.email,
            landingId,
            zoneId,
        ...
        await newDomainRecord.save();

        // Actualizar el documento del usuario agregando el dominio
        const user = await User.findOne({ email: decoded.email });
        if (user) {
            if (!user.domains) user.domains = [];
            user.domains.push({
                domainName,
                landingId,
                status: 'active',
        ...
        res.json({
            success: true,
            message: 'Dominio configurado exitosamente en Cloudflare.',
            domainName,
            nameServers
        });

    } catch (error) {
        console.error('Error general provisionando dominio:', error);
        res.status(500).json({ error: 'Error interno al procesar el dominio personalizado' });
    }
});

// ================= MIDDLEWARE DE RESOLUCIÓN DE DOMINIOS PROPIOS =================
app.use(async (req, res, next) => {
    const host = req.get('host'); // Ej: tumarca.com o landinggen.prestigecloser.com

    // Si la petición no es a tu dominio principal de sistema, evaluamos si es un dominio personalizado
    if (host && !host.includes(MAIN_DOMAIN) && !host.includes('localhost')) {
        try {
            const cleanHost = host.replace(/^www\./, ''); // Limpiar el www si lo trae
            const domainRecord = await Domain.findOne({ domainName: cleanHost, status: 'active' });

            if (domainRecord) {
                const landingData = await Landing.findOne({ landingId: domainRecord.landingId });
                if (landingData && landingData.htmlContent) {
                    res.setHeader('Content-Type', 'text/html; charset=utf-8');
                    return res.send(landingData.htmlContent);
                }
            }
        } catch (err) {
            console.error('Error resolviendo dominio personalizado:', err);
        }
    }
    next();
});
